...
> The package is based on [saber-dev-utils](https://github.com/facebook/create-react-app/tree/master/packages/saber-dev-utils) 

This package includes some utilities used by [saber-cli](https://github.com/wfatec/saber).<br>
Please refer to its documentation:

## Usage in saber-cli Projects

These utilities come by default with [saber-cli](https://github.com/wfatec/saber), which includes it by default. **You don’t need to install it separately in saber-cli projects.**

## Usage Outside of saber-cli

If you don’t use saber-cli, you may keep using these utilities. Their development will be aligned with saber-cli, so major versions of these utilities may come out relatively often. Feel free to fork or copy and paste them into your projects if you’d like to have more control over them, or feel free to use the old versions. Not all of them are React-specific, but we might make some of them more React-specific in the future.
...
var InterpolateHtmlPlugin = require('saber-dev-utils/InterpolateHtmlPlugin');
...
var path = require('path');
var HtmlWebpackPlugin = require('html-webpack-plugin');
var InlineChunkHtmlPlugin = require('saber-dev-utils/InlineChunkHtmlPlugin');
...
```js
var path = require('path');
var ModuleScopePlugin = require('saber-dev-utils/ModuleScopePlugin');
...
See [#186](https://github.com/facebook/saber/issues/186) for details.

```js
var path = require('path');
var WatchMissingNodeModulesPlugin = require('saber-dev-utils/WatchMissingNodeModulesPlugin');
...
module.exports = {
  ...
  plugins: [
    ...
    // See https://github.com/facebook/saber/issues/186
...
```js
var path = require('path');
var checkRequiredFiles = require('saber-dev-utils/checkRequiredFiles');
...
```js
var clearConsole = require('saber-dev-utils/clearConsole');
...
This is our custom ESLint formatter that integrates well with saber-cli console output.<br>
You can use the default one instead if you prefer so.

```js
const eslintFormatter = require('saber-dev-utils/eslintFormatter');
...
} = require('saber-dev-utils/FileSizeReporter');
...
var formatWebpackMessages = require('saber-dev-utils/formatWebpackMessages');
...
```
  const printBuildError = require('saber-dev-utils/printBuildError')
  try {
    build()
  } catch(e) {
...
```
saber
in /Users/developer/saber
```

```js
var getProcessForPort = require('saber-dev-utils/getProcessForPort');
...
On macOS, tries to find a known running editor process and opens the file in it. It can also be explicitly configured by `REACT_EDITOR`, `VISUAL`, or `EDITOR` environment variables. For example, you can put `REACT_EDITOR=atom` in your `.env.local` file, and saber-cli will respect that.
...
```js
var path = require('path');
var openBrowser = require('saber-dev-utils/openBrowser');
...
module.exports = {
  ...
  entry: [
    ...
    'saber-dev-utils/webpackHotDevClient',
    'src/index',
  ],
  // ...
};
...
```js
const getCSSModuleLocalIdent = require('saber-dev-utils/getCSSModuleLocalIdent');
...
```js
var getCacheIdentifier = require('saber-dev-utils/getCacheIdentifier');

getCacheIdentifier('prod', ['saber-dev-utils', 'chalk']); // # => 'prod:saber-dev-utils@5.0.0:chalk@3.0.0'
```
